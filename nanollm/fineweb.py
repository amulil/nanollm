...
# init the tokenizer
enc = AutoTokenizer.from_pretrained("Undi95/Meta-Llama-3-8B-hf")
bot = enc.bos_token_id
...
def tokenize(doc):
    ...
    tokens = [bot] # the special <|beginoftext|> token for all documents
    tokens.extend(enc.encode(doc["text"], add_special_tokens=False))
    tokens.extend([eot]) # the special <|endoftext|> token for all documents
